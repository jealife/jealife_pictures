    ...
    if (authLoading || loading) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-white dark:bg-zinc-950">
                <Loader2 className="w-10 h-10 animate-spin text-gray-200 dark:text-zinc-700" />
            </div>
        );
    }
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <button onClick={() => router.back()} className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors text-gray-500 dark:text-zinc-400">
                            <ArrowLeft className="w-6 h-6" />
                        </button>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-100">Modifier les détails</h1>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-12">
                    ...
                    <div className="space-y-4">
                        <div className="aspect-4/5 rounded-xl overflow-hidden bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 shadow-sm">
                            <img src={photoUrl} className="w-full h-full object-cover" alt="Preview" />
                        </div>
                        <p className="text-xs text-gray-400 dark:text-zinc-500 text-center italic">Aperçu de l&apos;image originale</p>
                    </div>
                    ...
                    <div className="space-y-12">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-900 dark:text-zinc-100">Titre de la photo</label>
                                <input
                                    type="text"
                                    value={formData.title}
                                    onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 focus:border-black dark:focus:border-white focus:ring-1 focus:ring-black dark:focus:ring-white outline-hidden transition-all"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-gray-900 dark:text-zinc-100">Ville</label>
                                    <input
                                        type="text"
                                        value={formData.city}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-900 dark:text-zinc-100">Contexte</label>
                                <textarea
                                    rows={4}
                                    value={formData.description}
                                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 focus:border-black dark:focus:border-white focus:ring-1 focus:ring-black dark:focus:ring-white outline-hidden transition-all resize-none"
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {formData.tags.map(tag => (
                                        <span key={tag} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 rounded-lg text-sm font-medium animate-in fade-in zoom-in-95 duration-200">
                                            {tag}
                            ...
                            {status.message && (
                                <div className={`p-4 rounded-xl flex items-center gap-3 text-sm animate-in slide-in-from-top-2 duration-300 ${status.type === 'success' ? 'bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-400 border border-green-100 dark:border-green-900' : 'bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-900'}`}>
                            ...
                            <div className="pt-4 flex gap-4">
                                <button
                                    type="submit"
                                    disabled={saving}
                                    className="flex-1 py-4 bg-black dark:bg-white text-white dark:text-black rounded-xl font-bold hover:bg-gray-800 dark:hover:bg-zinc-200 transition-all shadow-lg active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                ...
                                <Link
                                    href={`/photos/${id}`}
                                    className="px-8 py-4 bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-zinc-700 transition-all active:scale-95"
                                >
                                    Annuler
                                </Link>
                            </div>
                        ...
                        {/* Delete Section */}
                        <div className="pt-12 border-t border-red-100 dark:border-red-900/50">
                            <div className="bg-red-50/30 dark:bg-red-950/30 rounded-2xl border border-red-100 dark:border-red-900/50 p-8">
                                <h3 className="text-lg font-bold text-red-900 dark:text-red-400 mb-2">Supprimer l&apos;image</h3>
                                ...
                                {!showDeleteConfirm ? (
                                    <button
                                        ...
                                        className="text-red-600 dark:text-red-400 font-bold text-sm hover:underline flex items-center gap-2"
                                    >
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <p className="text-sm font-bold text-red-600 dark:text-red-400">Êtes-vous absolument sûr ? Cette action est irréversible.</p>
                                        <div className="flex gap-3">
                                            ...
                                            <button
                                                ...
                                                className="px-4 py-2 bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 rounded-lg text-sm font-bold hover:bg-gray-200 dark:hover:bg-zinc-700 transition-all active:scale-95"
                                            >
                                                Annuler
                                            </button>
                                        </div>
