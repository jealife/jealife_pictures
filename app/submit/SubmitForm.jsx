...
function ItemCard({ item, index, countries, dbTopics, onPatch, onRemove, onGenerate, disabled }) {
    ...
    // Suggestions de lieux (Photon/OpenStreetMap). Le code pays renvoyé au
    // passage remplit le menu « Pays » sans effort, ce qui alimente le
    // classement géographique du site.
    useEffect(() => {
        if (item.location.trim().length < 2) return;

        const timer = setTimeout(async () => {
            try {
                const response = await fetch(
                    `https://photon.komoot.io/api/?q=${encodeURIComponent(item.location)}&limit=5`
                );
    ...
    const visiblePlaces = showPlaces && item.location.trim().length >= 2 ? places : [];
            ...
            <div className="flex flex-col sm:flex-row">
                ...
                <div className="flex-1 p-5 space-y-3">
                    ...
                    <div className="relative">
                        ...
                        {/* Le pays déduit est montré, pas redemandé : sans ce
                            rappel, une déduction fausse partirait en base sans
                            que personne ne la voie. Le corriger se fait dans
                            « Plus de détails ». */}
                        {countryName && (
