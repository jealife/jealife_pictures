...
export default function AdminSettingsPage() {
    ...
    return (
        <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-zinc-100 mb-1">Réglages</h2>
                ...
                {MODES.map(({ key, label, description }) => (
                    <button
                        ...
                        className={`w-full text-left p-5 border rounded-2xl transition-all disabled:opacity-60 ${
                            mode === key ? "border-black dark:border-white bg-gray-50 dark:bg-zinc-800" : "border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-zinc-600"
                        }`}
                    >
                        <div className="flex items-center justify-between mb-2">
                            <span className="font-bold text-gray-900 dark:text-zinc-100">{label}</span>
                            {mode === key && <CheckCircle2 className="w-5 h-5 text-black dark:text-white" />}
                        </div>
            ...
            {saved && <p className="mt-6 text-sm text-emerald-600 dark:text-emerald-400 font-medium">Réglage enregistré.</p>}
        </div>
    );
}
