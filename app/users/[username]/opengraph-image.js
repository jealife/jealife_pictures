...
export default async function OpenGraphImage({ params }) {
    ...
    if (profile?.id) {
        ...
        if (photoUrl) {
            try {
                ...
                // `res.ok` ne suffit pas : un CDN en panne peut répondre 200
                // avec une page d'erreur HTML à la place de l'image.
    ...
    try {
        return await renderImage(bgStyle, logoData, displayName);
    } catch (error) {
        console.error("OpenGraph image (profil) : rendu impossible, repli sans image.", error?.message || error);
        return renderImage(
            { background: "linear-gradient(135deg, #0b3d2e 0%, #0f4423 25%, #0f172a 65%, #000 100%)" },
            null,
            displayName
...
async function renderImage(bgStyle, logoData, displayName) {
